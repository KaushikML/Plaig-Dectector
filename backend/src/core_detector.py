...
# ─────────── Gemini model selection (robust) ───────────
# Use GEMINI_MODEL in .env to control. We default to a newer flash model name, but
# will fall back automatically if not available in the current SDK/API version.
GEMINI_MODEL = os.getenv("GEMINI_MODEL", "gemini-2.0-flash")

def get_gemini_model():
    """Return a working GenerativeModel or None if Gemini is unavailable."""
    if not GOOGLE_API_KEY:
        return None

    name = GEMINI_MODEL.strip()
    ...
    try:
        ...
        if DEBUG:
            print(f"[Gemini] Failed to init model '{name}': {e}")
    ...
    try:
        ...
        # Prefer flash/pro if present
        for pref in ("flash", "pro"):
            for mname in candidates:
                if pref in mname:
                    if DEBUG:
                        print(f"[Gemini] Fallback model: {mname}")
                    return genai.GenerativeModel(mname)

        if candidates:
            if DEBUG:
                print(f"[Gemini] Fallback model: {candidates[0]}")
            return genai.GenerativeModel(candidates[0])

    except Exception as e:
        if DEBUG:
            print(f"[Gemini] list_models fallback failed: {e}")
...
# ─────────── Local embeddings (SentenceTransformers / E5) ───────────
# Default: intfloat/e5-large-v2 (free, runs locally). Override via E5_MODEL_NAME in .env.
_E5_MODEL_NAME = os.getenv("E5_MODEL_NAME", "intfloat/e5-large-v2")
_e5_model = None
...
def get_e5_model():
    ...
    if _e5_model is None:
        ...
        if DEBUG:
            print(f"[E5] Loading model: {_E5_MODEL_NAME} on {device}")
        _e5_model = SentenceTransformer(_E5_MODEL_NAME, device=device)
    return _e5_model

def embed(t: str) -> List[float]:
    """Return a normalized embedding vector for text using E5."""
    if t in _vec:
        return _vec[t]

    model = get_e5_model()

    # E5 performs best with task prefixes
    text = f"passage: {t.strip()}"
...
def detect_ai(txt):
    model = get_gemini_model()
    if model is None:
        return None, 'Gemini model unavailable'
    prompt = (
